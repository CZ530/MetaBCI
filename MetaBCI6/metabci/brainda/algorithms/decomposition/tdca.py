...
class FBTDCA(FilterBankSSVEP, ClassifierMixin):
    ...
    #     return labels
    def predict(self, X: ndarray, threshold: float =0):
        features = self.transform(X)
        print(features)
        ...
        if self.filterweights is None:
            ...
            features = np.mean(features, axis=1)
            print("Averaged features:", features)

        max_confidence = np.max(features, axis=1)
        predicted_index = np.argmax(features, axis=1)

        # 应用置信度阈值：如果最大值小于 threshold，就返回 -1 表示“不确定”
        labels = np.where(
            max_confidence >= threshold,
            self.classes_[predicted_index],
            -1  # 或其他特殊值，如 0
        )
        return labels
