...
def train_model(X, y, srate=1000):
    ...
    freqs = np.arange(8, 20, 2)
    Yf = generate_cca_references(freqs, srate=256, T=1.9, n_harmonics=3)
    model = FBTDCA(filterbank, padding_len=3, n_components=3,
...
class FeedbackWorker(ProcessWorker):
    ...
    def pre(self):
        ...
        while not self._exit:
            if self.outlet.wait_for_consumers(0.1):
                break
        print('Connected')
    ...
    def consume(self, data):
        ...
        udp_addr2Stim = ('192.168.10.2', 8000)
        ####################
        ...
        #############
        udp_socket2Stim.sendto(p_labels, udp_addr2Stim)
        ###############
...
if __name__ == '__main__':
    ...
    ns = NeuroScan(
        device_address=('192.168.0.142', 4000),
        srate=srate,
        num_chans=9)
