...
def train_model(X, y, algorithms, srate=1000):
    ...
    all_freq_list = np.arange(8, 15, 1)

    if algorithms == 'TDCA':
        freqs = all_freq_list
        Yf = generate_cca_references(freqs, srate=250, T=1, n_harmonics=1)
        model = FBTDCA(filterbank, padding_len=2, n_components=1,
                       filterweights=np.array(filterweights))

        model = model.fit(X, y, Yf)
    elif algorithms == 'eTRCA':
        model = FBTRCA(filterbank=filterbank, n_components=2, ensemble=True, filterweights=np.array(filterweights), n_jobs=-1)
        model = model.fit(X, y)
...
if __name__ == '__main__':
    ...
    #cnts = 1
    if len(sys.argv) >= 2:
        train_bdf = sys.argv[1]
    else:
        train_bdf = r"F:\META BCIbeijing\1.bdf"  # 默认训练文件，自己可以改

    if not os.path.exists(train_bdf):
        raise FileNotFoundError(f"离线训练文件不存在: {train_bdf}")

    run_files = [train_bdf]

    print("当前离线训练文件:", train_bdf)
    #runs = list(range(1, cnts + 1))
    # 修改：直接使用整数索引切片
    pick_chs = ['P3', 'PZ', 'P4',  'POZ','PO6', 'PO3', 'PO4', 'O1', 'O2']
    #pick_chs = ['FC3', 'FCZ', 'FC4', 'C3', 'C4', 'CP3', 'CP4']
    ...
    ns = NanoEEGAmplifier(
        ...
        srate=srate,
        num_chans=64  # 修改：必须是32，否则在线接收数据切片时会因为通道数不够而越界
    )
